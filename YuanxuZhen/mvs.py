...
class FeatureNet(nn.Module):
    def __init__(self):
        ...
        self.gru = nn.GRU(32,16,2,batch_first=True,bidirectional=True)
    ...
    def forward(self, x):
        ...
        # interpolate 上下采样函数，根据给定 size 或 scale_factor，
        # 上采样或下采样输入数据input.上采样算法有：nearest, linear(3D-only), bilinear(4D-only), trilinear(5D-only).
        ...
        #print(feature.shape)
        # 特征输入至gru结构时，需要重新变换维度，从Batch*C*H*W变换至Batch*H*W*C
        feature=feature.view(feature.shape[0],-1,32)
        # 将特征作为GRU单元的输入，由于GRU输出为元祖{A,B}的形式，因此取0位置的输出信息
        output=self.gru(feature)[0]
        # 将GRU的输出重新变换为原先表示形式，即Batch*C*H*W的形式
        output = output.view(feature.shape[0],128,-1,32)
        # permute将对应维度进行变换
...
class GRU_Attention(nn.Module):
    ...
    # gru_output : [batch_size, n_step, n_hidden * num_directions(=2)], F matrix
...
class CostGRURegNet(nn.Module):

      def __init__(self):
      
          super(CostGRURegNet,self).__init__()
          self.gru1=torch.nn.GRU(32,16,batch_first=True)
          self.gru2=torch.nn.GRU(16,8,batch_first=True)
          self.gru3=torch.nn.GRU(8,4,batch_first=True)
          self.gru4=torch.nn.GRU(4,1,batch_first=True)
          self.attention=GRU_Attention(1,1)
      
      def forward(self,x):
          x=x.view(x.shape[0],-1,32)
          print(x.shape)
          x=self.gru4(self.gru3(self.gru2(self.gru1(x)[0])[0])[0])[0]
          x=self.attention(x)
          x=x.view(1,1,192,128,160)
          return x
...
class CostRegNet(nn.Module):
    ...
    def forward(self, x):
        print("begin shape {}".format(x.shape))
        conv0 = self.conv0(x)
        print("conv0 shape {}".format(conv0.shape))
        conv2 = self.conv2(self.conv1(conv0))
        print("conv2 shape {}".format(conv2.shape))
        conv4 = self.conv4(self.conv3(conv2))
        print("conv4 shape {}".format(conv4.shape))
        x = self.conv6(self.conv5(conv4))
        print("conv6 shape {}".format(x.shape))
        ...
        print("prob forward {}".format(x.shape))
        x = self.prob(x)
        print("prob {}".format(x.shape))
        return x
...
class MVSNet(nn.Module):
    def __init__(self, refine=True):
        ...
        self.cost_regularization = CostGRURegNet()
        if self.refine:
...
def mvsnet_loss(depth_est_1,intrinsics,extrinsics,imgs,mask_photometric,outputs_feature):
    ...
    for i in range(len(src_img)):
        ...
        loss_reproj = loss_reproj + compute_seg_loss(sampled_img_src, ref_color, mask) * 1.0

    loss_sum = 0.4 * loss_photo + 0.4 * loss_reproj + 0.2 * loss_ssim + 0.0067 * loss_s + 0.0067*loss_perceptual
